...
If "kind" is "question": fill in "answer_to_user" with EXACTLY ONE short sentence (roughly 15 words or fewer) genuinely answering what they asked — grounded in this Department and the conversation so far, not a generic non-answer — optionally followed by ONE short sentence (also roughly 15 words or fewer) handing back to the original question. Two short sentences maximum, total — no chaining extra clauses onto either one with em dashes, semicolons, or "and"/"but". Do not literally repeat the original question's text — the app re-shows it separately.`

const ACTIONS_SCOPE_GUARD = `Scope: the user just finished a short guided reflection on this Department. Based on everything they said, suggest 2-3 small, concrete, achievable actions — not generic advice.
